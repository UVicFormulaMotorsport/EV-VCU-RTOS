...
extern volatile uint32_t adc_buf1[5]; // ADC1 - high priority readings
...
extern uint16_t adc1_pack_curr; //Measurement Via BSPD

extern volatile uint32_t adc_buf2[7]; // ADC2 - lower priority readings
extern uint16_t adc2_steering_pos;
extern uint16_t adc2_CoolantTemp1;
extern uint16_t adc2_CoolantTemp2;
extern uint16_t adc2_CoolantFlow;


extern volatile uint32_t adc_buf3[4];
extern uint16_t adc3_damper_FL;
extern uint16_t adc3_damper_FR;
extern uint16_t adc3_damper_RL;
extern uint16_t adc3_damper_RR;
...
void MX_ADC3_Init(void)
{
  ...
  hadc3.Init.ExternalTrigConv = ADC_SOFTWARE_START;
  hadc3.Init.DataAlign = ADC_DATAALIGN_RIGHT;
  hadc3.Init.NbrOfConversion = 4; //ADC3(PC0, PC1, PC2, PC3)
...
void StartADCTask(void *argument) {
    ...
    for (;;) {
        ...
        if(HAL_ADC_Start_DMA(&hadc3, (uint32_t *)adc_buf3, 7)!= HAL_OK){
